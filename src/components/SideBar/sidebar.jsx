    ...
    <div className="h-full flex flex-col">
      ...
      <div className="w-fit mx-auto p-2">
        <div className="flex items-center space-x-2 md:space-x-3">
          ...
          <span className="text-xl font-semibold text-gray-900 hidden md:inline">Base</span>
        </div>
      </div>
          ...
          {menuItems.map((item, index) => {
            ...
            return (
              <li key={index}>
                ...
                >
                  <div className="flex items-center justify-center space-x-0 md:justify-start md:space-x-3">
                    <item.icon className={`w-5 h-5 ${isActive ? 'text-blue-600' : 'text-gray-500'}`} />
                    <span className={`${isActive ? 'text-blue-600' : 'text-gray-700'} hidden md:inline`}>
                      {item.label}
                    </span>
                  </div>
                  {item.badge && (
                    <span className="bg-red-500 text-white text-xs font-semibold px-2 py-1 rounded-full min-w-[20px] text-center hidden md:inline">
                      {item.badge}
                    </span>
                  )}
                </Link>
      ...
      <div className="px-4 pb-6">
        <div className="bg-gray-50 rounded-xl p-4 mb-4 hidden md:block">
          <button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-4 rounded-lg text-sm">
            Upgrade Now
          </button>
        </div>
        <div className="flex items-center space-x-2 md:space-x-3 p-3 rounded-lg hover:bg-gray-50">
